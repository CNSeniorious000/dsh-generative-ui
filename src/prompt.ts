...
- **These five are the whole set — \`fs\`, \`ai\`, \`exec\`, \`chat\`, \`state\` — and a sixth you reason your way to does not exist.** If what you need is not one of the five, it does not exist under a plausible-sounding name either. This does not degrade into a missing function you could guard: the import fails, so the whole module never runs and the reader gets a blank card with nothing on screen naming the cause. If what you want is not on this list, build it out of what is.
