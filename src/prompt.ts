...
- **The React import line comes first, before anything else in the file.** A card that opens with a \`const\` table or a \`type\` and then calls \`useState\` throws \`useState is not defined\` at render — it compiles, mounts, and shows nothing. Write the import before you write the data:
