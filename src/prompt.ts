...
- **A style key written twice keeps only the last one.** \`style={{ padding: 4, ..., padding: "8px 12px" }}\` is not merged — the second wins and the first is discarded silently, which is how a card ends up ignoring the spacing you carefully set at the top of the object. Long style objects are where this happens; read the whole object before adding to it.
