...
- \`import { readFile, writeFile, readdir } from "$dsh/fs"\` reads and writes the workspace, under **the session's own access mode** — the same fence the model's own file tools run behind, so a read-only session refuses the write rather than pretending. Reach for it when the data belongs to the workspace (a file the user can also open, edit and commit); \`localStorage\` is still right for a canvas's own private state.
- \`import { streamText } from "$dsh/ai"\` runs a model call from inside the card, streaming characters, on the app's own model and credentials. Use it when the *content* is the variable part — the recipe, the itinerary, the five candidate names — and not when the data is fixed (a converter, a timer): calling a model to produce constants is worse than writing them down.
