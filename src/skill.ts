...
- **You are a component on someone else's page.** Your root is a normal node inside the chat column or the panel — nothing isolates you. No \`position: fixed\`, no \`100vw\`/\`100vh\`, no portals into \`document.body\`, no global listeners you don't remove. Overlays go in a \`relative\` wrapper you own with \`absolute inset-0\`.
...
- **Icons must name the thing beside them.** \`Sparkles\`, \`Wand\`, \`Bot\`, \`Zap\` as decoration say "an AI made this" and nothing else. A heading that reads fine without an icon doesn't get one. Prefer no icon to a decorative one.
...
Names you half-remember are the main failure mode: a wrong export is not a typo, it is an \`undefined\` component and a blank render. When you are not certain a name exists, look it up before you write the code rather than after it breaks.`;
