...
beforeEach(() => {
  ...
  (globalThis as any).document = {
    addEventListener: (type: string, fn: (event: any) => void) => { if (type === "pointerdown") listeners.push(fn) },
    removeEventListener: (type: string, fn: (event: any) => void) => { listeners = listeners.filter((l) => l !== fn) },
  };
});
...
describe("dismissOnOutsidePointer", () => {
  ...
  test("a press outside closes it", () => {
    let closed = 0;
    dismissOnOutsidePointer(true, anchorOf(), () => { closed += 1 });
  ...
  test("a press on the toggle inside the anchor does not", () => {
    ...
    let closed = 0;
    dismissOnOutsidePointer(true, anchorOf(toggle), () => { closed += 1 });
    press(toggle);
  ...
  test("a menu with no anchor yet still closes on any press", () => {
    let closed = 0;
    dismissOnOutsidePointer(true, null, () => { closed += 1 });
    press({ id: "anything" });
    expect(closed).toBe(1);
  });
  ...
  test("the disposer unsubscribes", () => {
    let closed = 0;
    const dispose = dismissOnOutsidePointer(true, anchorOf(), () => { closed += 1 });
    dispose?.();
    expect(listeners).toHaveLength(0);
