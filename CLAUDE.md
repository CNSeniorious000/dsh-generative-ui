...
- **Closing a canvas must leave a way back.** Dismissal only hides — the file is still on disk and still in the transcript — so without an entry point the work is stranded and the only recovery is making the model write it again. `CanvasLauncher` takes the panel's place at the right edge. Note that the repaint signature has to carry the hidden count as well: closing the last canvas changes nothing about the visible list, so a signature built from the visible canvases alone never repaints and the launcher never appears.
