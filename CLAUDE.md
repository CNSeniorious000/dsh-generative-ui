...
- **The headless browser used for verification cannot reach esm.sh.** `curl` gets a 200 from the same URL that `import()` inside the page fails on, so a card using any third-party package renders as a blank surface *in the probe* while working fine for the user. I read one such blank as "streaming charts are broken here" before checking. Verify third-party-package behaviour in the user's own browser, or not at all — and when a probe shows an empty card, `import()` the package by hand before believing anything about the code.
